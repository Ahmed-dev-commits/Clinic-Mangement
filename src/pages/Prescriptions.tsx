...
export function PrescriptionsPage() {
  ...
  const [ultrasoundReport, setUltrasoundReport] = useState('');
  const [examAtHospital, setExamAtHospital] = useState('');
  const [examAtHome, setExamAtHome] = useState('');
  ...
  const generatePrescriptionText = (): string => {
    ...
    const complaintText = complaint ? `C/O: ${complaint}.` : '';
    const historyText = history ? `History: ${history}.` : '';
    const examText = examination ? `O/E: ${examination}.` : '';
    ...
    return `Patient ${selectedPatient.name} (Age ${selectedPatient.age}). ${complaintText} ${historyText} ${examText} Diagnosed with ${diagnosis}. Prescribed ${medicineList}. ${labList} ${notesText} ${precautionsText}`.trim();
  };
  ...
  const handleEditDraft = (rx: Prescription) => {
    ...
    setUltrasoundReport(rx.ultrasoundReport || '');
    setExamAtHospital(rx.examAtHospital || '');
    setExamAtHome(rx.examAtHome || '');
  ...
  const handleDownloadPDF = (rx: Prescription) => {
    ...
    if (rx.complaint || rx.history || rx.examination) {
      ...
      if (rx.complaint) {
        doc.text('C/O:', margin + 4, detailY);
        doc.setTextColor(...textColor);
        doc.setFont('helvetica', 'normal');
        doc.text(rx.complaint, margin + 15, detailY);
        detailY += 6;
      }
      ...
      if (rx.history) {
        doc.setTextColor(...primaryColor);
        doc.setFont('helvetica', 'bold');
        doc.text('History:', margin + 4, detailY);
        doc.setTextColor(...textColor);
        doc.setFont('helvetica', 'normal');
        doc.text(rx.history, margin + 20, detailY);
        detailY += 6;
      }
      ...
      if (rx.examination) {
        doc.setTextColor(...primaryColor);
        doc.setFont('helvetica', 'bold');
        doc.text('O/E:', margin + 4, detailY);
        doc.setTextColor(...textColor);
        doc.setFont('helvetica', 'normal');
        doc.text(rx.examination, margin + 15, detailY);
      }
    ...
    // ============ REPORTS SECTION ============
    if (rx.ultrasoundReport || rx.examAtHospital || rx.examAtHome) {
      ...
      if (rx.ultrasoundReport) {
        doc.setTextColor(...primaryColor);
        doc.setFont('helvetica', 'bold');
        doc.text('USG:', margin + 4, reportY);
        doc.setTextColor(...textColor);
        doc.setFont('helvetica', 'normal');
        doc.text(rx.ultrasoundReport, margin + 15, reportY);
        reportY += 5;
      }

      if (rx.examAtHospital) {
        doc.setTextColor(...primaryColor);
        doc.setFont('helvetica', 'bold');
        doc.text('Hospital Exam:', margin + 4, reportY);
        doc.setTextColor(...textColor);
        doc.setFont('helvetica', 'normal');
        doc.text(rx.examAtHospital, margin + 25, reportY);
      }
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="form-section">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>Examination (O/E)</Label>
                <Textarea
                  value={examination}
                  onChange={(e) => setExamination(e.target.value)}
                  placeholder="e.g., BP 120/80, Pulse 72"
                  rows={1}
                />
              </div>
              <div>
                <Label>Ultrasound / Reports</Label>
                <Input
                  value={ultrasoundReport}
                  onChange={(e) => setUltrasoundReport(e.target.value)}
                  placeholder="e.g., USG Abdomen Normal"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>Exam at Hospital</Label>
                <Input
                  value={examAtHospital}
                  onChange={(e) => setExamAtHospital(e.target.value)}
                  placeholder="e.g., Ecg done"
                />
              </div>
              <div>
                <Label>Exam at Home</Label>
                <Input
                  value={examAtHome}
                  onChange={(e) => setExamAtHome(e.target.value)}
                  placeholder="e.g., Sugar monitoring"
                />
              </div>
            </div>
